...
def label(state):
	...
	firstOpositeValue = 1
	i = 0
	while state[(i+1)%len(state)] == value:
		i = (i+1)%len(state)
		firstOpositeValue += 1
		if i == 0:
			label.append(str(firstOpositeValue-1))
			return(label)
	value = state[firstOpositeValue]
	numberOfValues = 0
	for repetition in range(len(state)):
		if state[(repetition+firstOpositeValue)%len(state)] == value:
			numberOfValues += 1
		else:
			value = state[(repetition+firstOpositeValue)%len(state)]
...
# for label, amplitude in amplitudes(groupsOfStates(4)).items():
# 	print(label, " - ", amplitude)
