	...
	for n in range(7,8,1):
		...
		with open(f'maximize{n}.nb', 'w') as mathematica, open('template.txt', 'r') as template:
			mathematica.write(	'Print["started"]\n\n'
								'amplitude[β_, γ_, β2_, γ2_, β3_, γ3_] := ' + amplitude + '\n' +
						  		'probability[β_, γ_, β2_, γ2_, β3_, γ3_] := Abs[amplitude[β, γ, β2, γ2, β3, γ3]]^2\n')
		...
		while t < 600:
			try:
				cpi = subprocess.run(
					["math", "-noinit", "-script", f'maximize{n}.nb'],
				...
				if "ok" in e.stdout.decode().split('\n'):
					...
					t += 30
					print(e)
					print("retrying")		# remove('maximize.nb')
