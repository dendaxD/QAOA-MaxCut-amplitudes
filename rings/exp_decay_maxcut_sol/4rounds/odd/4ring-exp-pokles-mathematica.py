...
def create_amplitude(n):
	...
	for l in labels:	
		...
		for l2 in labels:
			for c in computation_constants[l][l2].keys():
				amplitude += "Exp[" + str(-c) + " I γ3" + "]*(" + amplitudes2[l2] + ")*("
				for numOfSin in computation_constants[l][l2][c].keys():
					m = str( computation_constants[l][l2][ c ][ numOfSin ] )
					if numOfSin == n:
						amplitude += ( m + " (I Sin[β3])^" + str(n) )
					elif numOfSin == 0:
						amplitude += ( m + " Cos[β3]^" + str(n) )
					else:
						amplitude += ( m + " (I Sin[β3])^" + str(numOfSin) + " Cos[β3]^" + str(n - numOfSin) )
					amplitude += " + "
				amplitude = amplitude[:-3] + ") + "
		#[:-3] to remove additional ' + '
	...
	for l2 in labels:
		for c in computation_constants[l][l2].keys():
			amplitude += "Exp[" + str(-c) + " I γ4" + "]*(" + amplitudes3[l2] + ")*("
			for numOfSin in computation_constants[l][l2][c].keys():
				m = str( computation_constants[l][l2][ c ][ numOfSin ] )
				if numOfSin == n:
					amplitude += ( m + " (I Sin[β4])^" + str(n) )
				elif numOfSin == 0:
					amplitude += ( m + " Cos[β4]^" + str(n) )
				else:
					amplitude += ( m + " (I Sin[β4])^" + str(numOfSin) + " Cos[β4]^" + str(n - numOfSin) )
				amplitude += " + "
			amplitude = amplitude[:-3] + ") + "
	 #[:-3] to remove additional ' + '
...
#MAXIMIZING P₀₁...₀₁
with open('results.txt', 'w') as data, open('minimizing average energy', 'w') as data2:
	for n in range(9,10,2):
		...
		with open(f'maximize{n}.nb', 'w') as mathematica, open('template.txt', 'r') as template:
			mathematica.write(	'amplitude[β_, γ_, β2_, γ2_, β3_, γ3_, β4_, γ4_] := ' + amplitude + '\n\n' +
						  		'probability[β_, γ_, β2_, γ2_, β3_, γ3_, β4_, γ4_] := Abs[amplitude[β, γ, β2, γ2, β3, γ3, β4, γ4]]^2\n')
