...
#UPPER BOUND
with open('data/upper bound', 'w') as data:
	for n in range(3,20,2):
		...
		for ecut, ampl in create_cut_to_amplitude_dict(n).items():
			...
			with open('maximize.nb', 'w') as mathematica, open('../templates/template-upper-bound.txt', 'r') as template:
				mathematica.write(	'Print["started"]\n\n' +
									'term[β_] := ' + ampl + '\n' +
							  		'termmagnitude[β_] := Abs[term[β]]^2;\n\n')
			...
			while t < 600:
				try:
					cpi = subprocess.run(
						["math", "-noinit", "-script", 'maximize.nb'],
						stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=t,
						check=True
					)
					...
					# print(e)
					if "ok" in e.stdout.decode().split('\n'):
						print("probably ok")
						break
					else:
						t += 30
						print(e)
						print("retrying")
			remove(f'maximize.nb')

		with open('maxima.txt', 'r') as f:
			bound = sum(map(lambda x: float(x.strip().replace('*^','e').replace(' + 0.*I','')),f.readlines()))
		print(n, ' ', bound, ' ', bound**2)
		data.write(f'{n}\t{bound**2}\n')
		remove('maxima.txt')

#MAXIMIZING P₀₁...₀₁₀
with open('data/maximizing p₀₁...₀₁₀', 'w') as data, open('data/minimizing average energy', 'w') as data2:
	for n in range(3,18,2):
		...
		with open('maximize.nb', 'w') as mathematica, open('../templates/template.txt', 'r') as template:
			mathematica.write(	'Print["started"]\n\n' +
								'amplitude[β_, γ_] := ' + amplitude + '\n' +
						  		'probability[β_, γ_] := Abs[amplitude[β, γ]]^2;\n\n')
			for line in template:
				mathematica.write(line)
		#here we run the mathematica file and then we remove it
		#here we run the mathematica file and then we remove it
		...
		with open('maxima.txt', 'r') as f, open('maxima2.txt', 'r') as f2:
			prob1 = sum(map(lambda x: float(x.strip().replace('*^','e').replace(' + 0.*I','')),f.readlines()))
			prob2 = sum(map(lambda x: float(x.strip().replace('*^','e').replace(' + 0.*I','')),f2.readlines()))
...
system('gnuplot logscale-rings.gnuplot')