		...
		for c in constants.keys():
			...
			for exponent, coefficient in constants[c].items():
				if exponent == n:
					...
					amplitude += f"{coefficient} (-I Sin[x])^{exponent} Cos[x]^{n - exponent}"
				amplitude += " + "
	...
	for l in labels:
		...
		for l2 in labels:
			for c in computation_constants[l][l2].keys():
				amplitude += f"Exp[{-c} I y]*({amplitudes[l2]})*("
...
def main():
	...
	#here we are creating result.txt: [type <-> Pmax <-> angles] table
	...
	with open('result.txt', 'w') as f:
		f.write(tabulate(results, headers=['Name', 'Max Probability', "b1 (X)", "g1 (ZZ)"]))
