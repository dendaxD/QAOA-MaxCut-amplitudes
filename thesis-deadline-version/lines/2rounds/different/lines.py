...
for numberOfQubits in range(start,end+1):
	...
	for label1, amplitude in amplitudes(amplitudes(states)[1], "2")[0].items():
		print(label1, " - ", amplitude)
	ampls = amplitudes(amplitudes(states)[1], "2")[0]
	for label1, amplitude in ampls.items():
		...
		with open("MathematicaFiles/" + str(numberOfQubits) + "v" + label1 + ".nb", "w") as f, open("templates/useful-template.nb", "r") as template:
			...
			for line in template:
				f.write(line)
			f.write("\nManipulate[Plot3D[f,{c,-n,n},{d,-n,n}, PlotRange -> All],{e,-n,n},{f,-n,n}]\n")
			f.write("\nManipulate[ContourPlot[function[x, y], {x, -n, n}, {y, -n, n}, PlotLegends -> Automatic, Contours -> 30],{e,-n,n},{f,-n,n}]\n")
